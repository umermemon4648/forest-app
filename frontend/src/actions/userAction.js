...
// confirm user
export const confirmUser = (userEmail) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${apiBaseUrl}/api/v1/resendConfirmation`,
      { email: userEmail },
      config
    );

    dispatch({ type: CONFIRM_USER_SUCCESS, payload: data.message });
  } catch (error) {
    dispatch({
...
export const loadUser = () => async (dispatch) => {
  try {
    ...
    if (!token) {
      // If there's no token, dispatch a failure action
    ...
    const { data } = await axios.get(`${apiBaseUrl}/api/v1/me`, config);
...
export const updatePassword = (passwords) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${apiBaseUrl}/api/v1/password/update`,
      passwords,
      config
    );
...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${apiBaseUrl}/api/v1/password/forgot`,
      email,
      config
    );
...
export const resetPassword = (token, passwords) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${apiBaseUrl}/api/v1/password/reset/${token}`,
      passwords,
      config
    );
...
export const getUserDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${apiBaseUrl}/api/v1/admin/user/${id}`);
...
export const updateUser = (id, userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${apiBaseUrl}/api/v1/admin/user/${id}`,
      userData,
      config
    );
